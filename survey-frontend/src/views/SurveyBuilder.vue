    ...
    <el-card class="box-card">
        ...
        <el-form label-width="80px" size="small">
          ...
          <template v-if="q.type === 'single' || q.type === 'multiple'">
            <el-form-item label="选项设置">
              ...
              >
                ...
                />
                <el-button type="danger" link @click="removeOption(q, oIndex)"
                  >删除</el-button
                >
              </div>
          ...
          <template v-if="q.type === 'multiple'">
            <el-form-item label="选择限制">
              最少选
              <el-input-number
                v-model="q.constraints.min_select"
                :min="1"
                style="width: 100px; margin: 0 10px"
              />
              项， 最多选
              <el-input-number
                v-model="q.constraints.max_select"
                :min="1"
                style="width: 100px; margin: 0 10px"
              />
              项
          ...
          <template v-if="q.type === 'number'">
            <el-form-item label="数值限制">
              最小值
              <el-input-number
                v-model="q.constraints.min_value"
                style="width: 120px; margin: 0 10px"
              />
              最大值
              <el-input-number
                v-model="q.constraints.max_value"
                style="width: 120px; margin: 0 10px"
              />
              <el-checkbox v-model="q.constraints.is_integer"
                >必须为整数</el-checkbox
              >
            </el-form-item>
          </template>
          <template v-if="q.type === 'text'">
            <el-form-item label="字数限制">
              最少
              <el-input-number
                v-model="q.constraints.min_length"
                :min="0"
                style="width: 100px; margin: 0 10px"
              />
              字， 最多
              <el-input-number
                v-model="q.constraints.max_length"
                :min="1"
                style="width: 100px; margin: 0 10px"
              />
              字
          ...
          <el-form-item label="跳转逻辑" style="margin-bottom: 0">
            ...
            >
              ...
              >
                <template
                  v-for="(targetQ, tIndex) in survey.questions"
                  :key="targetQ.q_id"
                >
                  <el-option
                    v-if="targetQ.q_id !== q.q_id"
                    :label="
                      `Q${tIndex + 1}. ` + (targetQ.title || '未命名题目')
                    "
                    :value="targetQ.q_id"
                  />
                </template>
              ...
              <el-button type="danger" link @click="removeJumpLogic(q, lIndex)"
                >删除</el-button
              >
            </div>

            <div style="width: 100%">
              <el-button type="primary" link @click="addJumpLogic(q)"
                >+ 添加跳转规则</el-button
              >
            </div>
          </el-form-item>
      ...
      <div class="add-toolbar">
        <el-button type="primary" plain @click="addQuestion('single')"
          >+ 单选题</el-button
        >
        <el-button type="success" plain @click="addQuestion('multiple')"
          >+ 多选题</el-button
        >
        <el-button type="info" plain @click="addQuestion('text')"
          >+ 文本填空</el-button
        >
