        ...
        <el-form :model="formData" label-position="top" size="large">
          ...
          >
            <el-form-item :required="q.is_required">
              ...
              <el-radio-group v-if="q.type === 'single'" v-model="formData[q.q_id]" @change="evaluateJumpLogic">
                <el-radio v-for="opt in q.options" :key="opt" :value="opt">
                  {{ opt }}
                </el-radio>
              </el-radio-group>

              <el-checkbox-group v-else-if="q.type === 'multiple'" v-model="formData[q.q_id]" @change="evaluateJumpLogic">
          ...
          <div class="submit-action">
            <el-button type="primary" size="large" @click="submitResponse" :loading="submitting">
              提交答卷
            </el-button>
          </div>
        </el-form>
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    for (const logic of q.jump_logic) {
      if (q.type === 'single' && currentValue === logic.condition_value) {
        targetId = logic.target_q_id; break
      } else if (q.type === 'multiple' && currentValue.includes(logic.condition_value)) {
        targetId = logic.target_q_id; break
      } else if (q.type === 'number' && currentValue === Number(logic.condition_value)) {
        targetId = logic.target_q_id; break
      }
    }
